...
def save_logs_to_gcs():
    ...
    print(
        f"Logs successfully saved to GCS at gs://{GCS_BUCKET}/{log_filepath}")
...
def move_existing_files_to_archive(table):
    ...
    for file in existing_files:
        ...
        storage_client.bucket(GCS_BUCKET).copy_blob(
            source_blob, storage_client.bucket(GCS_BUCKET), destination_blob.name)
        source_blob.delete()
...
def extract_and_save_to_landing(table, load_type, watermark_col):
    try:
        last_watermark = get_latest_watermark(
            table) if load_type.lower() == "incremental" else None
        log_event(
            "INFO", f"Latest watermark for {table}: {last_watermark}", table=table)
        ...
        log_event(
            "SUCCESS", f"Successfully extracted data from {table}", table=table)
        ...
        blob.upload_from_string(df.toPandas().to_json(
            orient="records", lines=True), content_type="application/json")

        log_event(
            "SUCCESS", f"JSON file successfully written to gs://{GCS_BUCKET}/{JSON_FILE_PATH}", table=table)

        # Insert audit entry
        audit_df = spark.createDataFrame(
            [("hospital_b_db", table, load_type, df.count(),
              datetime.datetime.now(), "SUCCESS")],
            ["datasource", "tablename", "load_type",
